...
/* Borrowed from Xailer: Copyright OZ Software & Xailer

Revision: 4389
Author: ignacio
Date: martes, 8 de mayo de 2007 12:29:46
Message:
* source\internal\isqlite.c
  + Soporte de ordenacion WinLocaleNoCase y WinLocaleCase

Revision: 8749
Author: iozuniga
Date: lunes, 20 de noviembre de 2023 21:29:12
Message:
* source\internal\isqlite.c
  + Nuevas funciones internas: ansi2utf8() y utf82ansi()
  + Nuevas colaciones: UTF8Case y UTF8NoCase

*/

static int fnCompAnsiNoCase(void *pCtx, int nA, const void *zA, int nB, const void *zB)
{
   HB_SYMBOL_UNUSED( pCtx );
#ifdef _WINDOWS_
	return CompareStringA( LOCALE_SYSTEM_DEFAULT, NORM_IGNORECASE, zA, nA, zB, nB ) - 2;
#else
   HB_SYMBOL_UNUSED( nA );
   HB_SYMBOL_UNUSED( nB );
   return hb_stricmp( zA, zB );
#endif
}

static int fnCompAnsiCase(void *pCtx, int nA, const void *zA, int nB, const void *zB)
{
   HB_SYMBOL_UNUSED( pCtx );
#ifdef _WINDOWS_
	return CompareStringA( LOCALE_SYSTEM_DEFAULT, 0, zA, nA, zB, nB ) - 2;
#else
   HB_SYMBOL_UNUSED( nA );
   HB_SYMBOL_UNUSED( nB );
   return strcmp( zA, zB );
...
static int fnCompUtf8NoCase(void *pCtx, int nA, const void *zA, int nB, const void *zB)
{
   PHB_CODEPAGE cdp = hb_cdpFind( "UTF8EX" );
   HB_SYMBOL_UNUSED( pCtx );
   if( cdp )
      return hb_cdpicmp(  zA, nA, zB, nB, cdp, HB_FALSE );
   else
      return strcmp( zA, zB );
}

static int fnCompUtf8Case(void *pCtx, int nA, const void *zA, int nB, const void *zB)
{
   PHB_CODEPAGE cdp = hb_cdpFind( "UTF8EX" );
   HB_SYMBOL_UNUSED( pCtx );
   if( cdp )
      return hb_cdpcmp(  zA, nA, zB, nB, cdp, HB_FALSE );
   else
      return strcmp( zA, zB );
}
   ...
   if( hb_fsFileExists( pszdbName ) || hb_parl( 2 ) )
   {
      if( sqlite3_open( pszdbName, &db ) == SQLITE_OK )
      {
         ...
         hb_sqlite3_ret( hbsqlite3, HB_SQLITE3_DB );
			sqlite3_create_collation( db, "WinLocaleAnsiNoCase", SQLITE_UTF8, NULL, fnCompAnsiNoCase );
			sqlite3_create_collation( db, "WinLocaleAnsiCase", SQLITE_UTF8, NULL, fnCompAnsiCase );
         sqlite3_create_collation( db, "UTF8Case", SQLITE_UTF8, NULL, fnCompUtf8Case );
         sqlite3_create_collation( db, "UTF8NoCase", SQLITE_UTF8, NULL, fnCompUtf8NoCase );
      }
      else
      {
         sqlite3_close( db );
...
HB_FUNC( SQLITE3_COLUMN_TEXT )
{
   ...
   {
      int iIndex = hb_parni( 2 ) - 1;
      char *pStr = (char *) sqlite3_column_text( pStmt, iIndex );
      int   iLen = sqlite3_column_bytes( pStmt, iIndex );

      if( *( pStr + iLen - 1 ) == '\0' )
         iLen--;

      hb_retstrlen_utf8( pStr, iLen );
   }
   else
      hb_errRT_BASE_SubstR( EG_ARG, 0, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );
}
