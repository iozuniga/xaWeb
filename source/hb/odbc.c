...
HB_FUNC( SQLCOLUMNDEFAULT )  /* hDbc, cTable, cField */
{
   SQLHDBC hDbc = hb_SQLHDBC_par( 1 );
   SQLHSTMT hStmt;
   ...
   if( hStmt )
   {
      ...
      retcode = SQLColumns(hStmt, NULL, 0, NULL, 0, (SQLCHAR*) hb_parc( 2 ),
                           hb_parclen( 2 ), (SQLCHAR*) hb_parc( 3 ), hb_parclen( 3 ) );
      ...
      if ( SQL_SUCCEEDED( retcode ) ) {
         if ( SQL_SUCCEEDED( SQLFetch( hStmt ) ) ) {
            ...
            SQLLEN nLen = 0;
            buffer[ 0 ] = '\0';
            SQLGetData( hStmt, 13, SQL_C_CHAR, buffer, 129, &nLen );
            if ( nLen > 0 ) {
...
HB_FUNC( SQLBEGINTRANSACTION )  /* hDbc */
{
   SQLHDBC hDbc = hb_SQLHDBC_par( 2 );
...
HB_FUNC( SQLCOMMIT )  /* hDbc */
{
   SQLHDBC hDbc = hb_SQLHDBC_par( 1 );

   if( hDbc )
      {
      hb_retni( SQLEndTran( SQL_HANDLE_DBC, hDbc, SQL_COMMIT ) );
...
HB_FUNC( SQLROLLBACK )  /* hDbc */
{
   SQLHDBC hDbc = hb_SQLHDBC_par( 2 );

   if( hDbc )
      {
      hb_retni( SQLEndTran( SQL_HANDLE_DBC, hDbc, SQL_ROLLBACK ) );
      SQLSetConnectAttr( hDbc, SQL_ATTR_AUTOCOMMIT, (SQLPOINTER)SQL_AUTOCOMMIT_ON,
                         SQL_IS_UINTEGER);
      }
   else
...
HB_FUNC( SQLCATALOGS )
{
   ...
   if( hStmt )
      {
      hb_retni( SQLTables( hStmt, (SQLCHAR*)SQL_ALL_CATALOGS, SQL_NTS, (SQLCHAR*)"",
...
HB_FUNC( SQLSCHEMAS )
{
   ...
   if( hStmt )
      {
      hb_retni( SQLTables( hStmt, (SQLCHAR*)"", SQL_NTS,
...
{
   SQLHSTMT hStmt = hb_SQLHSTMT_par( 1 );
   const char * catalog = hb_parc( 2 );
   const char * schema = hb_parc( 3 );

   if( hStmt )
      {
      hb_retni( SQLTables( hStmt, (SQLCHAR*)catalog,
                           SQL_NTS, (SQLCHAR*)schema,
...
HB_FUNC( SQLMORERESULTS )  /* hStmt --> nRetCode */
{
   SQLHSTMT hStmt = hb_SQLHSTMT_par( 1 );
...
HB_FUNC( SQLGETHBTYPE )  /* nType, @xDefault*/
{
   SQLSMALLINT iType = hb_parni( 1 );
   char cType[ 1 ];
