...
/* NOTE: To satisfy MySQL headers. */
   ...
   #include "mysql.h"
#else
   #include "Z:\MariaDB\32\include\mysql.h"
#endif
...
HB_FUNC( MYSQL_INIT ) /* MYSQL * mysql_init() */
{
   MYSQL * mysql;
...
HB_FUNC( MYSQL_REAL_CONNECT ) /* MYSQL * mysql_real_connect( MYSQL *, char * host, char * user, char * password, char * db, uint port, char *, uint flags ) */
{
   ...
   const char * szHost = hb_parc( 2 );
   const char * szUser = hb_parc( 3 );
   const char * szPass = hb_parc( 4 );
   unsigned int port   = ( unsigned int ) hb_parnidef( 5, MYSQL_PORT );
   unsigned int flags  = ( unsigned int ) hb_parnidef( 6, 0 );
   HB_BOOL lSSLVerify  = hb_parldef( 7, HB_FALSE );

   mysql_options( mysql, CLIENT_MULTI_RESULTS, NULL );
   mysql_options( mysql, CLIENT_MULTI_STATEMENTS, NULL );
   mysql_options( mysql, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, (const void *) &lSSLVerify );
...
HB_FUNC( MYSQL_CLOSE ) /* MYSQL * mysql_close( MySQL *) */
{
   MYSQL * mysql = hb_MYSQL_par( 1 );
   mysql_close( mysql );
   hb_ret();
...
HB_FUNC( MYSQL_NEXT_RESULT ) /* int mysql_query( MYSQL *, char * ) */
{
   MYSQL * mysql = hb_MYSQL_par( 1 );

   if( mysql )
      hb_retnl( ( long ) mysql_next_result( mysql ) );
   else
      hb_errRT_BASE( EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS );
}
...
HB_FUNC( MYSQL_OPTIONS_BOOLEAN )
{
   MYSQL * mysql = hb_MYSQL_par( 1 );
   HB_BOOL lValue = hb_parl( 3 );
   mysql_options( mysql, hb_parnl( 2 ), (char*) &lValue );
   hb_retl( lValue );
}

HB_FUNC( MYSQL_OPTIONS_SSL_VERIFY )
{
   MYSQL * mysql = hb_MYSQL_par( 1 );
   HB_BOOL lValue = hb_parl( 2 );
   mysql_options( mysql, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, (char*) &lValue );
   hb_retl( lValue );
}
