...
CLASS WDocMain FROM WDoc
   ...
   DATA oDS    AS CLASS WOdbcDataSource
...
METHOD SelectRange() CLASS WDocMain
   ...
   WITH OBJECT ::oDS
      cQuery := :BuildSqlSt( "SELECT TOP ? * FROM customer WHERE ID>?", nChunk, nOffset )
...
//   Engine:lDebug := .t.
   ...
   WITH OBJECT ::oDS := WOdbcDataSource():New()
      :cDSN := "xaweb" // you must create this DSN. Consult ODBC docs.
      :Connect()
      ::nRecs := :QueryValue( "SELECT Count(*) FROM customer", 0 )
      oQuery  := :Query( "SELECT TOP 1 * FROM customer" )
      aData   := oQuery:aData
      aHeader := oQuery:Header()
      LogDebug( :cConnectOut )
   END WITH
   ...
   WITH OBJECT WDiv():New( Self )
      :AddClass( "container" )
      ECHO "<h4>xaWeb - Materialize - Access ODBC sample (xaWeb DS)</h4>"
