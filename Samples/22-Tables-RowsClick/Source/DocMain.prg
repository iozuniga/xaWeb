...
/*
Note: Only the first Javascript method is supported on Xailer Win32 local execution
*/
...
CLASS WDocMain FROM WDoc
   ...
   METHOD Act_EventMethod( hParams, hEvent )
   METHOD Act_RowClick( hParams, hEvent )

   METHOD Srv_FetchJson( hParams )
...
METHOD CreateDoc() CLASS WDocMain
   ...
   /*
   Javascript Event en table row click:
   The parameter event holds two important new values:
   - detail.recno: Record number
   - detail.fields: Array of fields of the selected row
   */
   ...
   Customers( @aData, @aHeader, 5  )

   WITH OBJECT ::oFetch1 := WFetch():New()
      :cUrl      := ::Service( "Srv_FetchJson" )
      :cTargetId := "rowInfo"
      :cSourceId := "table1"
      :cCallBack := "fetchDataRowClick"
      :AddJsParam( "table1", "recno" )
      :cContentType := "application/json"
   END WITH

   WITH OBJECT ::oFetch2 := WFetch():New()
   ...
   ECHO "<h2>xaWeb - Table with row click events</h2>" INTO Self

   WITH OBJECT ::oLbx := WSelect():New( Self )
      :aItems := { {"1", "Resolved in JavaScript"},;
                   {"2", "Resolved in Harbour"},;
                   {"3", "Resolved in Harbour+Fetch (application/json)"},;
                   {"4", "Resolved in Harbour+Fetch (application/javascript)"};
                 }
      :cId    := "listbox1"
      :Create()
      :OnChange := "Act_EventMethod"
   END WITH

   WITH OBJECT ::oText := WParagraph():New (Self )
      :cId := "rowInfo"
...
METHOD Act_EventMethod( hParams, hEvent ) CLASS WDocMain

   SWITCH ::oLbx:cValue
   CASE "1"
      ::oTable:OnRowClick := "jsrowClick"
      EXIT
   CASE "2"
      ::oTable:OnRowClick := "Act_RowClick"
      EXIT
   CASE "3"
      ::oTable:OnRowClick := ::oFetch1
      EXIT
   CASE "4"
      ::oTable:OnRowClick := ::oFetch2
      EXIT
   OTHERWISE
      ::oTable:OnRowClick := "jsrowClick"
   END SWITCH
...
//------------------------------------------------------------------------------
/*
Harbour Event on table row click:
The parameter hEvent holds two important new values:
- detail-recno: Record number
- detail-fields: Array of fields of the selected row
*/

METHOD Act_RowClick( hParams, hEvent ) CLASS WDocMain

   LOCAL cText

   TEXT INTO cText
   <p>Recno:  <RECNO> (resolved in Harbour)</p>
   <p>( Sort position is lost. Consider using a javascript function or a WFetch() object.</p>
   ENDTEXT

   cText := StrTran( cText, "<RECNO>", ToString( hEvent[ 'detail-recno' ] ) )
...
//------------------------------------------------------------------------------
/*
This method is callled inside a Fetch operation (cContentType := "application/json").
You can only return a Json data. That's all! You can not modify anything on the
web page. That part must be done in the callback JavaScript function
function fetchDataRowClick(element, oParams)
*/

METHOD Srv_FetchJson( hParams ) CLASS WDocMain

RETURN HB_JsonEncode( hParams )

//------------------------------------------------------------------------------

METHOD Srv_Magic( hParams ) CLASS WDocMain

   ::oText:cText := "<p>Recno: " + hParams[ 'detail-recno' ] + ;
      " Resolved in Harbour+Fetch (application/javascript) <b>xaWeb magic!</b>"
