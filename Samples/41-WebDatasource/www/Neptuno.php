 ...
 *
 * Copyright 2012, 2025 Ignacio Ortiz de Zuñiga
...
$cDBUserName = 'neptuno';
$cDBPassword = '!9maF5f14';
$cCryptKey   = 'change this value';
...
if ( isset( $_POST[ 'data' ] ) ) {
   ...
   XA_Exit( 'POST data empty: Error on PostRequest statement' );
   return;
}
...
if ( XA_Validate($cPostData, $cCryptKey ) != $nValid ) {
   XA_Exit( 'Invalid POST request: Validation error' );
   return;
}
...
if ( isset( $_GET[ 'dbName' ] ) ) {
   ...
   if ( $oDB->connect_errno ) {
      XA_Exit( 'Failed to connect to MySQL: (' . $oDB->connect_errno . ') ' . $oDB->connect_error );
      return;
   }
   //$oDB->set_charset('latin1'); // Not used on xaWeb
} else {
   $oDB = NULL;
}
...
if ( isset( $_GET[ 'command' ] ) ) {
   ...
   case 'SENDMAIL':
      ...
      break;
   default:
      XA_AddError( '#Error: Invalid command: '.$_GET[ 'command' ] );
   }
}
else {
   XA_AddError( 'Command not defined' );
}
...
function XA_SendMail( $cData ) {
   ...
   try {
      ...
      $mail->SMTPDebug  = 0;
      $mail->SMTPSecure = 'ssl';
      $mail->Host       = 'cien-com-es.correoseguro.dinaserver.com';
      $mail->SMTPAuth   = TRUE;
      $mail->Port       = 465;
      $mail->Username   = 'fredy@cien.com.es';
      $mail->Password   = '7y7=Catorce';

      $mail->setFrom( 'fredy@cien.com.es', 'fredy' );
...
function xa_ErrorHandler($code, $msg, $file, $line) {
   $file = basename($file);
   XA_Exit( "On line $line of file $file. Error: $msg ($code)" );
}
