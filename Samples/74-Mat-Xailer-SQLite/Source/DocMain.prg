/*
This sample uses the SQLITE engine from Xailer. You must include the following
libraries:
- [xailer] CGILinux
- [xailer] SQLiteLinux (for linux version)

Benefits of Xailer version over the Contribution Harbour version:
- Encription
- Some Harbour alike implemented functions on SQL statements

This module was just a temporary module until we have our own SQLITE engine.
Please do not use it. :-)

Encription is not important since the database is on the server side where the
user can not access.
*/
...
CLASS WDocMain FROM WDoc
   ...
   DATA oDS    AS CLASS WXailerSqliteDataSource
...
METHOD CreateDoc() CLASS WDocMain
   ...
   ELSE
      cPath := HB_DirBase() + "data\"
      // copy customer.db to c:\xampp\cgi-bin\data
   ENDIF
#endif

   WITH OBJECT ::oDS := WXailerSqliteDatasource():New()
      :cConnect := cPath + "customer.db"
      :Connect( :cConnect )
   ...
   WITH OBJECT WDiv():New( Self )
      :AddClass( "container" )
      ECHO "<h4>xaWeb - Materialize & Xailer - SQLite Table pagination</h4>"
