/*
This sample uses the SQLITE engine from Harbour contribution. It does not require
Xailer at all, but it lacks important features present on Xailer version:
 - Encription
...
CLASS WDocMain FROM WDoc
   ...
   DATA oDS    AS CLASS WSqliteDataSource
...
METHOD CreateDoc() CLASS WDocMain
   ...
   WITH OBJECT ::oDS := WSqliteDatasource():New()
      :cDatabase := cPath + "customer.db"
      :Connect()
...
//      LogDebug( oQuery:SqlInsert( oRecord ) )
   END WITH
   ...
   WITH OBJECT WDiv():New( Self )
      :AddClass( "container" )
      ECHO "<h4>xaWeb - Materialize - SQLite sample (xaWeb DS)</h4>"
